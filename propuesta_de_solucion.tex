...
\begin{tikzpicture}[
    ...
    \node[model, below=1.2cm of canonical] (features) {\textbf{Contrato de atributos}\\Censo 2024 + OSM + distancia};
    \node[model, below=0.75cm of features] (evaluation) {\textbf{Entrenamiento y evaluación}\\particiones por origen, baselines y CPC};
...
\end{tikzpicture}
\caption{Procedimiento de Santiago: un núcleo común de viajes precede a los adaptadores espaciales y a la evaluación. Fuente: Elaboración propia a partir de los contratos F2-C, F2-G y F2-I.}
...
La cohorte primaria exige una secuencia de etapas coherente, extremos observados con la calidad indicada por el contrato, zonas de origen y destino disponibles y \texttt{factor\_expansion} no negativo. El factor de expansión de \texttt{viajes} es el peso principal para los flujos expandidos; el conteo sin expansión se conserva como análisis de sensibilidad. La Tabla~\ref{tab:contrato_datos} distingue procedencia y función de los insumos para evitar confundir pesos, coordenadas y variables explicativas.

\begin{table}[htbp]
\centering
\small
\caption{Contrato de datos y procedencia de las variables usadas por el procedimiento. Fuente: Elaboración propia a partir de F2-C y F2-I.}
\label{tab:contrato_datos}
...
OSM Chile 2024-01-01 & Polígonos, vías y puntos del extracto histórico. & Aporta coberturas y densidades del contrato \texttt{paper19}; un cero representa ausencia cartografiada, no prueba de ausencia real. \\
\bottomrule
\end{tabularx}
\end{table}
...
La rama Zona 777 usa las zonas de origen y destino auditadas en el viaje canónico y la geometría oficial publicada por DTPM. La rama H3 transforma las coordenadas válidas desde EPSG:32719 a WGS84 con \texttt{always\_xy=True} y asigna cada extremo mediante \texttt{latlng\_to\_cell}. La política de borde conserva la celda H3 completa observada: los viajes se recortan por el dominio aprobado, pero las huellas de las celdas no se recortan ni se reparte flujo por superficie.

H3 es un sistema jerárquico de índices geoespaciales. Sus áreas y distancias varían geográficamente, existen pentágonos y la vecindad no puede resumirse como seis vecinos para toda celda. Por ello, H3 no elimina el MAUP: la comparación trata la discretización espacial como una fuente de sensibilidad empírica. El análisis de factibilidad consideró H3-r3 a H3-r8; H3-r7 y H3-r8 pasaron al piloto junto con Zona 777. La Tabla~\ref{tab:representaciones} presenta las representaciones operativas de la cohorte común, sin inferir superioridad entre ellas.

\begin{table}[htbp]
\centering
\small
\caption{Representaciones espaciales operativas después de la exclusión común. Fuente: elaboración propia a partir de ADR-006 y F2-I.}
...
Una zona o celda es una unidad OD. Un \textit{tile} es un bloque mayor, de 15.000 m de lado en EPSG:32719, indexado por $\operatorname{floor}(x/15000)$ y $\operatorname{floor}(y/15000)$. Se fijaron diez \textit{tiles} comunes y una agenda determinista con semilla 20260909: siete para entrenamiento, dos para validación y uno para prueba. La partición se aplica al origen; todos los destinos del dominio aprobado, incluidos los de otros \textit{tiles} o particiones, permanecen candidatos y los flujos cruzados se conservan. La Figura~\ref{fig:objetos_espaciales} distingue estos objetos y evita equiparar un \textit{tile} con una unidad OD.
...
\begin{tikzpicture}[x=0.78cm,y=0.78cm,font=\scriptsize]
    ...
    \node[align=center] at (2.6,3.65) {Área territorial\\aprobada};
...
\subsection{Contrato de atributos y vector de entrada}
\label{sec:features_osm}

El contrato principal \texttt{paper19} usa 19 variables por ubicación y, para un par OD, forma un vector de 39 entradas: 19 del origen, 19 del destino y una distancia. Incluye población censal, cinco coberturas de uso de suelo, tres densidades de longitud vial y cinco familias de servicios, cada una medida mediante densidad de puntos y cobertura de áreas. \texttt{macro12\_geometry20} se mantiene como alternativa diagnóstica, con 20 variables por ubicación y 41 entradas, pero no reemplaza el contrato principal del piloto.

\begin{equation}
    \mathbf{x}_{ij} = [\mathbf{a}_{i}^{(19)},\, \mathbf{a}_{j}^{(19)},\, d_{ij}],
    \label{eq:vector_entrada}
\end{equation}
donde $\mathbf{a}_{i}$ y $\mathbf{a}_{j}$ contienen los atributos externos de origen y destino, respectivamente, y $d_{ij}$ es la distancia geodésica calculada desde representantes espaciales en WGS84. La población se interpola desde Censo 2024 a las unidades; no se reemplaza por el flujo saliente observado.
...
Durante el entrenamiento se pueden tomar hasta \texttt{destinations\_per\_origin} destinos por origen, sin reposición. \texttt{positive\_destination\_fraction} reserva una cuota de destinos con flujo positivo y el resto se extrae uniformemente de los candidatos aún disponibles; esa fracción no permite concluir que los demás seleccionados sean negativos. La evaluación usa el soporte completo de destinos aprobados, no una muestra. En el piloto, los destinos de todo el dominio metropolitano se mantienen disponibles aunque correspondan a otro \textit{tile} o partición.
...
\end{equation}
Minimizar esta expresión equivale a maximizar la log-verosimilitud multinomial bajo el contrato de soporte y pesos definido. Su relación con la formulación de entropía de Wilson es conceptual y requiere restricciones adicionales; no se identifica automáticamente con maximizar la entropía de Wilson \cite{wilson1970entropy}.

\subsubsection{Baselines, particiones y métricas}
