...
\begin{itemize}
    \item Los \textbf{modelos físicos y estadísticos clásicos} (Secciones 3.1 y 3.2) ofrecen transparencia matemática, rigor explicativo y garantías de aditividad, pero fracasan al intentar modelar las complejidades y no linealidades del uso de suelo moderno al depender exclusivamente de la distancia y la masa poblacional como variables \textit{proxy} de oportunidad.
    \item Los \textbf{modelos avanzados basados en Deep Learning, series de tiempo temporales e imágenes satelitales} (Secciones 3.3 y 3.4) alcanzan una alta precisión predictiva y transferibilidad global, pero se estructuran como cajas negras inflexibles, opacas y computacionalmente costosas que limitan su utilidad para fundamentar políticas públicas explicables.
\end{itemize}

En este contexto metodológico, el uso del modelo \textbf{\textit{Deep Gravity}} \cite{simini2021deep} alimentado con datos de \textbf{OpenStreetMap (OSM)} y analizado mediante técnicas de explicabilidad \textbf{SHAP} \cite{lundberg2017shap} se posiciona como una solución de equilibrio o \textit{sweet spot} para el caso de Santiago de Chile. 
