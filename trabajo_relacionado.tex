...
Tanto el modelo de radiación como el modelo PWO comparten un supuesto implícito sobre el comportamiento del viajero. Utilizando la terminología introducida retrospectivamente por Liu y Yan \cite{liu2020intervening} para clasificar los mecanismos de selección de destino, el modelo de radiación asume una \textit{tendencia cautelosa}: el individuo elige el destino más cercano que supera el beneficio del origen. El modelo PWO también incorpora una lógica de competencia local. En ambos casos, la distancia juega un papel restrictivo en la selección.

Investigaciones posteriores mostraron que este supuesto no aplica universalmente. En el ámbito de los procesos de evaluación y selección social, Sim et al. \cite{sim2015deliberate} identificaron un mecanismo alternativo al que denominaron \textit{tendencia exploratoria}: los individuos que buscan el mejor resultado posible no se restringen a las opciones más cercanas, sino que aceptan recorrer distancias mayores si el destino ofrece un beneficio suficientemente alto. Liu y Yan \cite{liu2019opportunity} trasladaron formalmente este concepto al dominio de la movilidad espacial y propusieron el \textit{Modelo de Selección Prioritaria de Oportunidades} (Opportunity Priority Selection), en el que un individuo en $i$ elige el destino $j$ si su beneficio supera tanto al del origen como al de todas las oportunidades intervinientes:
\begin{equation}
...
En el contexto latinoamericano, los antecedentes de aplicación de modelos computacionales para la generación de flujos son escasos. Spadon et al. \cite{spadon2019reconstructing} reconstruyeron redes de commuting en la región metropolitana de São Paulo utilizando \textit{random forests} e indicadores urbanos municipales, demostrando la viabilidad de enfoques basados en aprendizaje automático para ciudades de la región. Sin embargo, su trabajo emplea arquitecturas de baja complejidad, no incorpora \textit{amenities} de OpenStreetMap como variables explicativas y no incluye técnicas de explicabilidad. No se ha identificado en la literatura ninguna aplicación de modelos de aprendizaje profundo con explicabilidad para la generación de flujos OD en ciudades latinoamericanas, lo que constituye la brecha que la presente memoria busca abordar.
...
En este contexto metodológico, el uso del modelo \textit{Deep Gravity} \cite{simini2021deep} alimentado con datos de OpenStreetMap (OSM) y analizado mediante técnicas de explicabilidad SHAP \cite{lundberg2017shap} se posiciona como una solución de equilibrio para el caso de Santiago de Chile.
