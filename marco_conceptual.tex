...
La elección de la entrada de referencia $\mathbf{x}'$ es un aspecto crítico del método. El vector nulo ($\mathbf{x}' = \mathbf{0}$) es una convención habitual en clasificación de imágenes, pero puede no ser representativo para variables de densidad urbana, que por definición nunca son cero en zonas habitadas. En esta memoria se evalúa el uso del vector promedio de la distribución de características como referencia alternativa, lo que permite interpretar las atribuciones como desviaciones respecto al comportamiento medio del territorio.

Los \textit{Integrated Gradients} se utilizan como técnica complementaria a SHAP para obtener atribuciones a nivel de par origen-destino específico, permitiendo identificar qué características son más relevantes para flujos particulares.
