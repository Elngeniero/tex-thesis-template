...
El modelo gravitatorio presenta limitaciones ampliamente documentadas \cite{liu2024interdisciplinary}. Su formulación impone una relación paramétrica monótona entre población, distancia y flujo, lo que restringe su capacidad de representar fenómenos no lineales. No incorpora variables del tejido urbano como puntos de interés, uso de suelo o infraestructura de transporte. Requiere además calibración manual para cada contexto geográfico, lo que limita su transferibilidad. La implicancia concreta de estas restricciones para el caso de Santiago se detalla en la Sección de Definición del Problema de esta memoria.
...
Simini et al. \cite{simini2012universal} validaron el modelo sobre datos de commuting del censo de Estados Unidos (2000). Los resultados muestran que el modelo de radiación supera al modelo gravitatorio en la reproducción de flujos observados, especialmente en viajes de corta distancia, que el modelo gravitatorio tiende a sobreestimar.
...
\begin{equation}
    ...
    \label{eq:cpc}
\end{equation}
La métrica CPC toma valores en $[0,1]$, donde 1 indica coincidencia exacta entre flujos generados y reales. Es equivalente al índice de Sørensen-Dice aplicado a flujos. Los resultados muestran que \textit{Deep Gravity} supera al modelo gravitatorio clásico en todos los contextos evaluados. Las mejoras en CPC global alcanzan hasta un 190\% en Inglaterra y un 1.066\% en el estado de Nueva York. El modelo también demostró transferibilidad geográfica. Entrenado en algunas ciudades del Reino Unido, mantiene un desempeño similar al evaluarse en ciudades no vistas durante el entrenamiento.

\subsubsection{Síntesis comparativa de modelos}

La Tabla~\ref{tab:comparacion_modelos} compara los modelos de generación de flujos presentados en este capítulo en cinco dimensiones clave para esta memoria: incorporación de variables del tejido urbano, ausencia de calibración paramétrica, aptitud para la escala intraurbana, compatibilidad con técnicas de explicabilidad (XAI), y existencia de validación empírica en el contexto latinoamericano.

\begin{table}[ht]
\centering
\caption{Comparación de modelos de generación de flujos de movilidad. \textbf{Sí}~= cumple la dimensión; \textbf{No}~= no cumple; \textit{Parc.}~= cumple parcialmente.}
\label{tab:comparacion_modelos}
\small
\begin{tabular}{lp{1.7cm}p{1.8cm}p{1.8cm}p{1.8cm}p{1.8cm}}
\hline
\textbf{Modelo} &
\textbf{Variables urbanas} &
\textbf{Libre de parámetros} &
\textbf{Escala intra\-urbana} &
\textbf{Explicable (XAI)} &
\textbf{Validado en LatAm} \\
\hline
Gravitatorio \cite{zipf1946p}
    & No & No & \textit{Parc.} & No & \textit{Parc.} \\
IO Model \cite{stouffer1940intervening}
    & No & No & \textit{Parc.} & No & No \\
Radiación \cite{simini2012universal}
    & No & Sí & No & No & No \\
Mod.\@ avanz.\@ IO \cite{yan2014population, liu2020intervening}
    & No & Sí & Sí & No & No \\
\textbf{Deep Gravity} \cite{simini2021deep}
    & \textbf{Sí} & No & \textbf{Sí} & \textbf{Sí} & $\dagger$ \\
\hline
\end{tabular}

\smallskip
\raggedright\footnotesize{$\dagger$~Esta memoria constituye la primera validación de \textit{Deep Gravity} en una metrópolis latinoamericana.}
\end{table}

\textit{Deep Gravity} es el único modelo que combina simultáneamente las tres capacidades centrales para esta investigación: incorpora variables geoespaciales del tejido urbano provenientes de OSM, es apto para la escala intraurbana, y admite análisis de explicabilidad mediante SHAP e Integrated Gradients. La ausencia de validación en el contexto latinoamericano constituye precisamente la brecha que esta memoria busca cubrir para el caso del Gran Santiago de Chile.
...
La principal fuente histórica de datos de movilidad en Santiago es la Encuesta Origen-Destino (EOD) del SECTRA\footnote{Secretaría de Planificación del Transporte, organismo dependiente del Ministerio de Transportes y Telecomunicaciones de Chile.}. Su última versión fue levantada en 2012, con actualizaciones parciales en 2017. Esta encuesta constituye la referencia canónica de flujos de movilidad en la ciudad. Sin embargo, su periodicidad de siete a diez años, combinada con el elevado costo de levantamiento, implica que los datos disponibles pueden no reflejar la movilidad actual, especialmente tras la reconfiguración de los patrones de desplazamiento producida por la pandemia de COVID-19.

Una fuente complementaria, de mayor resolución temporal, son los registros de validación de la tarjeta bip!, administrados por el DTPM\footnote{Directorio de Transporte Público Metropolitano.}. Estos datos registran cada validación en Metro y buses con información de paradero, zona y hora, lo que permite construir matrices OD a nivel de viaje individual. No obstante, no cubren etapas caminadas, viajes en taxi o colectivo pagados en efectivo, ni modos no motorizados.

Esta memoria utiliza el dataset de viajes y etapas de noviembre de 2024 del DTPM, que comprende aproximadamente 3{,}6 millones de viajes en un mes completo, con información de zona de inicio, zona de fin, modo de transporte, hora, factor de expansión y coordenadas estimadas de subida y bajada. Este dataset, de mayor resolución temporal y amplitud que la EOD, permite construir matrices OD a escala diaria o por período horario (punta mañana, punta tarde, fuera de punta). Constituye la principal fuente de flujos observados para el entrenamiento y evaluación del modelo \textit{Deep Gravity} en Santiago.
